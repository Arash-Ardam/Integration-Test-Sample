...
namespace TPG.SI.TadbirPay.URLTest.Abstractions
{
    public class IntegrationTestBase : IClassFixture<TadbirPayWebApplicationFactory>, IDisposable
    {
        ...
        protected HttpClient _IdpHttpClient;
        ...
        List<string> Scopes = new List<string>()
            {
                "openid",
                "profile",
                "TadbirPay.Withdrawal.Api.Scope",
                "TadbirPay.Administrator.Api.Scope",
        ...
        public async Task SetAccesstokenAsync(HttpClient httpClient)
        {
            _IdpHttpClient = _factory.CreateClient();

            //var discoveryDocumentRequest = new DiscoveryDocumentRequest()
            //{
            //    Address = "https://si-lab-idp.etadbir.com",
            ...
            //_discoveryDocument = await _IdpHttpClient.GetDiscoveryDocumentAsync(discoveryDocumentRequest);
            PasswordTokenRequest req = new PasswordTokenRequest()
            {
                Address = "https://si-lab-idp.etadbir.com/connect/token",
                ClientId = "TadbirPay.Api.Swagger",
                UserName = "Ali",
                Password = "Aa#123456",
                Scope = string.Join(" ", Scopes)
            };

            var tokenResponse = await _IdpHttpClient.RequestPasswordTokenAsync(req);
        ...
        protected async Task GetPaymentIdFromConnectionString() 
        {
            string result = string.Empty;

            var connectionString = "Server=.;Database=TPG.SI.TadbirPay;User Id=sa;Password=a`123456;TrustServerCertificate=True";

            using (var DbConnection = new SqlConnection(connectionString))
            {
                using (var command = DbConnection.CreateCommand())
                {
                    command.CommandText = "use [TadbirPay.TadbirPardaz] select Id from dbo.WithdrawalOrder where Name = 'test' ";
            ...
            LastTestDoneActionId = Guid.Parse(result);
        }

        public async Task<int> getTenantsCount()
        {
            var query = "use [TPG.SI.TadbirPay] select COUNT(*) from dbo.Tenant";
        ...
        public async Task ExecuteQueryAsync(string query)
        {
            using (var command = _dbConnection.CreateCommand())
            {
                ...
                //this.Tenants = (await command.ExecuteReaderAsync()).GetString(0);
                var result = await command.ExecuteScalarAsync();
                this.Tenants = (int)result;
        ...
        {
            var connectionString = "Server=172.16.30.144,1433;Database=TPG.SI.TadbirPay;User Id=si-user;Password=tadbir@123;TrustServerCertificate=True";
